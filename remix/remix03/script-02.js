...
function colorStopWords(passage) {
  ...
  for (i = 0; i < words.length; i++) { 
    ...
    if (stopArr.includes(word)) {
      HTMLstr += `<span class="words-space shake-hard" style="bottom: ${Math.random() * (30 - 0) + 0}px">${word}</span> `
      ...
      if (stopArr.includes(res.toLowerCase())) {
        ...
        HTMLstr += `<span class="words-stop shake-hard">${word}</span> `;
      }

    } else if (char == char.toUpperCase()) {
      if (stopArr.includes(word.toLowerCase())) {
        HTMLstr += `<span class="words-space" style="bottom: ${Math.random() * (30 - 0) + 0}px">${word}</span> `;
      } else {
        HTMLstr += `<span class="words-stop">${word}</span> `;
      }
    } else {
      HTMLstr += `<span class="words-stop">${word}</span> `;
    }
